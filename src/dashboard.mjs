...
.stat-card.unstable .stat-value{color:#f59e0b}
.stat-card.dead .stat-value{color:#ef4444}
.health-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:16px;margin-bottom:30px}
...
.uptime-bar{width:100%;height:8px;background:#1a1a3e;border-radius:4px;overflow:hidden;margin-top:4px}
...
.country-flag{font-size:18px;margin-left:4px}
.server-name{color:#a855f7;font-weight:600;font-size:12px}
@media(max-width:768px){.stats-grid{grid-template-columns:repeat(2,1fr)}.health-grid{grid-template-columns:1fr}.charts-grid{grid-template-columns:1fr}.stat-value{font-size:28px}.header h1{font-size:22px}}
...
<div class="container">
  ...
  <div class="health-grid" id="healthGrid">
    ...
    <div class="stat-card unstable"><div class="stat-icon">⚠️</div><div class="stat-value" id="unstableCount">-</div><div class="stat-label">ناپایدار (30-80%)</div><div class="uptime-bar"><div class="uptime-fill uptime-mid" id="unstableBar" style="width:0%"></div></div></div>
  ...
  <div class="table-card">
    <div class="table-header">
      ...
      <input type="text" class="search-box" id="searchBox" placeholder="جستجو (آدرس، پروتکل، کشور)...">
    </div>
    <div style="overflow-x:auto">
      <table>
        <thead><tr><th>#</th><th>وضعیت</th><th>نام سرور</th><th>پروتکل</th><th>آدرس سرور</th><th>پورت</th><th>کشور</th><th>پینگ</th><th>امتیاز</th></tr></thead>
...
function getFlag(code){return FLAGS[(code||'').toUpperCase()]||'🌐'}
function render(){
  if(!data)return;
  ...
  const h=data.health||{};
  const totalH=(h.healthyServers||0)+(h.unstableServers||0)+(h.deadServers||0)||1;
  document.getElementById('healthyCount').textContent=h.healthyServers||'-';
  document.getElementById('unstableCount').textContent=h.unstableServers||'-';
  document.getElementById('deadCount').textContent=h.deadServers||'-';
  document.getElementById('healthyBar').style.width=((h.healthyServers||0)/totalH*100)+'%';
  document.getElementById('unstableBar').style.width=((h.unstableServers||0)/totalH*100)+'%';
  document.getElementById('deadBar').style.width=((h.deadServers||0)/totalH*100)+'%';
...
function renderCharts(){
  ...
  new Chart(document.getElementById('protocolChart'),{type:'doughnut',data:{labels:Object.keys(proto).map(k=>k.toUpperCase()),datasets:[{data:Object.values(proto),backgroundColor:COLORS}]},options:{...chartDefaults,plugins:{legend:{position:'bottom',labels:{color:'#aaa',padding:12}}}}});
  const countries=Object.fromEntries(Object.entries(data.byCountry||{}).slice(0,12));
  new Chart(document.getElementById('countryChart'),{type:'bar',data:{labels:Object.keys(countries).map(c=>getFlag(c)+' '+c),datasets:[{label:'سرور',data:Object.values(countries),backgroundColor:COLORS.slice(0,Object.keys(countries).length)}]},options:{...chartDefaults,indexAxis:'y',plugins:{legend:{display:false}},scales:scaleDefaults}});
...
function renderTable(configs){
  ...
  if(!configs.length){tbody.innerHTML='<tr><td colspan="9" style="text-align:center;padding:40px;color:#555">داده‌ای موجود نیست</td></tr>';return}
  tbody.innerHTML=configs.map((c,i)=>{
    ...
    const latencyClass=typeof latency==='number'?(latency<200?'badge-green':latency<500?'badge-blue':latency<1000?'badge-yellow':'badge-red'):'badge-gray';
    const flag=c.flag||getFlag(c.country);
    const countryName=c.countryName||c.country||'?';
    const serverName='MRCODAD '+flag+' Node-'+String(i+1).padStart(3,'0');
    return '<tr><td>'+(i+1)+'</td><td><span class="status-dot '+cls+'"></span>'+(c.alive?'فعال':'غیرفعال')+'</td><td><span class="server-name">'+serverName+'</span></td><td><span class="badge '+protoBadge+'">'+proto.toUpperCase()+'</span></td><td style="direction:ltr;text-align:left;font-family:monospace;font-size:12px">'+(c.host||'-')+'</td><td>'+c.port+'</td><td><span class="country-flag">'+flag+'</span> '+countryName+'</td><td><span class="badge '+latencyClass+'">'+latency+(typeof latency==='number'?'ms':'')+'</span></td><td><strong>'+(c.score||0)+'</strong></td></tr>';
  }).join('');
}
document.getElementById('searchBox').addEventListener('input',function(){
  if(!data||!data.configs)return;
  const q=this.value.toLowerCase();
  const filtered=data.configs.filter(c=>(c.host||'').toLowerCase().includes(q)||(c.protocol||'').toLowerCase().includes(q)||(c.country||'').toLowerCase().includes(q)||(c.countryName||'').includes(q));
