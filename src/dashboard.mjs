...
<div class="header">
  ...
  <div class="nav">
    ...
    <button class="nav-btn" onclick="showSection('qr')">📱 QR</button>
    <button class="nav-btn" onclick="showSection('isp')">🔍 ISP</button>
    <button class="nav-btn" onclick="showSection('advisor')">🤖 مشاور</button>
...
<div class="container">
  ...
  <div class="section" id="sec-qr">
    <div class="map-container">
      <h3>📱 QR Connect — اسکن و وصل شو</h3>
      <p style="color:#888;font-size:12px;margin-bottom:12px">دوربین موبایل رو باز کنید و QR رو اسکن کنید</p>
      <div class="speed-grid" id="qrGrid"></div>
    </div>
  </div>
  <!-- ISP Detector -->
  <div class="section" id="sec-isp">
    <div class="map-container">
      <h3>🔍 تشخیص ISP و پیشنهاد</h3>
      <div id="ispInfo" style="margin-bottom:16px"></div>
      <div id="ispRecommendations"></div>
    </div>
  </div>
  <!-- AI Advisor -->
  <div class="section" id="sec-advisor">
    <div class="map-container">
      <h3>🤖 مشاور هوشمند کانفیگ</h3>
...
async function loadData(){try{const r=await fetch('results.json?'+Date.now());if(!r.ok)throw 0;data=await r.json();render();renderMap();renderSpeed();renderCensor();renderGame();renderQR();renderAdvisor();renderISP();}catch(e){document.getElementById('sec-overview').innerHTML='<div class="no-data"><p>داده‌ای موجود نیست...</p></div>';}}
function getFlag(c){return FLAGS[(c||'').toUpperCase()]||'🌐'}
function showSection(id){document.querySelectorAll('.section').forEach(s=>s.classList.remove('active'));document.getElementById('sec-'+id).classList.add('active');document.querySelectorAll('.nav-btn').forEach(b=>b.classList.remove('active'));event.target.classList.add('active');if(id==='map')renderMap();if(id==='qr')renderQR();if(id==='isp')renderISP();if(id==='advisor')renderAdvisor();}
...
// QR Connect
function renderQR(){if(!data)return;const grid=document.getElementById('qrGrid');if(!grid)return;const configs=(data.configs||[]).filter(c=>c.alive).slice(0,12);if(!configs.length){grid.innerHTML='<div class="no-data">داده‌ای نیست</div>';return}grid.innerHTML=configs.map((c,i)=>{const flag=getFlag(c.country);const subLink='https://raw.githubusercontent.com/MRcodad/dev-vibe-cli/main/dist/sub.txt';const qrUrl='https://api.qrserver.com/v1/create-qr-code/?size=200x200&data='+encodeURIComponent(subLink);const serverName='MRCODAD '+flag+' Node-'+String(i+1).padStart(3,'0');return '<div class="speed-card" style="cursor:pointer" onclick="window.open(\''+subLink+'\',\'_blank\')"><img src="'+qrUrl+'" alt="QR" style="width:120px;height:120px;border-radius:8px;margin-bottom:8px"><div style="font-size:12px;font-weight:600;color:#a855f7">'+serverName+'</div><div style="font-size:10px;color:#888">'+(c.host||'?')+':'+c.port+'</div><div style="margin-top:4px"><span class="badge badge-green">'+c.latency+'ms</span></div></div>'}).join('');}

// ISP Detector
async function renderISP(){const info=document.getElementById('ispInfo');const recs=document.getElementById('ispRecommendations');if(!info||!data)return;try{const r=await fetch('https://ipapi.co/json/');if(!r.ok)throw 0;const ipData=await r.json();const ip=ipData.ip||'?';const org=ipData.org||'نامشخص';const city=ipData.city||'?';const country=ipData.country_name||'?';info.innerHTML='<div style="background:#1a1a3e;border-radius:10px;padding:16px;border:1px solid #2a2a5a"><div style="font-size:18px;font-weight:700;margin-bottom:8px">🌐 اطلاعات اتصال شما</div><div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px"><div><div style="color:#888;font-size:11px">IP</div><div style="direction:ltr;font-family:monospace">'+ip+'</div></div><div><div style="color:#888;font-size:11px">ISP</div><div style="font-weight:600">'+org+'</div></div><div><div style="color:#888;font-size:11px">مکان</div><div>'+country+', '+city+'</div></div></div></div>';const configs=(data.configs||[]).filter(c=>c.alive);const isIran=ip&&(ip.startsWith('5.')||ip.startsWith('37.')||ip.startsWith('80.')||ip.startsWith('81.')||ip.startsWith('88.')||ip.startsWith('89.')||ip.startsWith('91.')||ip.startsWith('92.')||ip.startsWith('94.')||ip.startsWith('95.')||ip.startsWith('185.')||org.toLowerCase().includes('iran'));if(isIran){const recs_html=['<div style="margin-top:16px"><h3 style="color:#ccc;margin-bottom:12px">🎯 توصیه‌های ویژه ایران</h3><div style="display:grid;gap:8px">'];const topConfigs=configs.sort((a,b)=>(a.latency||9999)-(b.latency||9999)).slice(0,5);for(const c of topConfigs){const flag=getFlag(c.country);const isReality=(c.protocol==='vless');recs_html.push('<div style="background:#1a1a3e;border-radius:8px;padding:10px;border:1px solid #2a2a5a;display:flex;align-items:center;gap:10px"><span style="font-size:20px">'+flag+'</span><div style="flex:1"><div style="font-weight:600;font-size:12px">'+(c.host||'?')+':'+c.port+'</div><div style="font-size:10px;color:#888">'+c.protocol?.toUpperCase()+(isReality?' • REALITY ✅':'')+'</div></div><span class="badge badge-green">'+c.latency+'ms</span></div>');}recs_html.push('</div></div>');recs.innerHTML=recs_html.join('');}else{recs.innerHTML='<div style="text-align:center;padding:20px;color:#888">🌍 شما خارج از ایران هستید. اکثر سرورها برای شما مناسب‌اند.</div>';}}catch(e){info.innerHTML='<div style="color:#888;text-align:center;padding:20px">无法 دریافت اطلاعات ISP. لطفاً بعداً تلاش کنید.</div>';}}

// AI Advisor
function renderAdvisor(){const out=document.getElementById('advisorOutput');if(!out||!data)return;const configs=(data.configs||[]).filter(c=>c.alive);if(!configs.length){out.innerHTML='<div class="no-data">داده‌ای نیست</div>';return}const avgLat=configs.reduce((a,b)=>a+(b.latency||0),0)/configs.length;const bestConfigs=[...configs].sort((a,b)=>(a.latency||9999)-(b.latency||9999)).slice(0,3);const worstConfigs=[...configs].sort((a,b)=>(b.latency||0)-(a.latency||0)).slice(0,3);let internetQuality='';if(avgLat<150)internetQuality='⚡ سریع';
else if(avgLat<300)internetQuality='🟢 متوسط';
else if(avgLat<500)internetQuality='🟡 کند';
else internetQuality='🔴 خیلی کند';
const lines=['<div style="background:#1a1a3e;border-radius:10px;padding:16px;border:1px solid #2a2a5a;margin-bottom:16px">','<div style="font-size:18px;font-weight:700;margin-bottom:12px">🤖 تحلیل هوشمند</div>','<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-bottom:16px">','<div><div style="color:#888;font-size:11px">کیفیت اینترنت شما</div><div style="font-size:16px;font-weight:700">'+internetQuality+'</div></div>','<div><div style="color:#888;font-size:11px">میانگین پینگ</div><div style="font-size:16px;font-weight:700">'+Math.round(avgLat)+'ms</div></div>','<div><div style="color:#888;font-size:11px">سرورهای فعال</div><div style="font-size:16px;font-weight:700">'+configs.length+'</div></div>','</div>','</div>','<div style="background:#1a1a3e;border-radius:10px;padding:16px;border:1px solid #22c55e;margin-bottom:16px">','<div style="font-size:14px;font-weight:700;color:#22c55e;margin-bottom:8px">🏆 بهترین سرورها برای شما:</div>'];for(let i=0;i<bestConfigs.length;i++){const c=bestConfigs[i];const flag=getFlag(c.country);lines.push('<div style="display:flex;align-items:center;gap:8px;padding:6px;border-bottom:1px solid #1a1a3e"><span style="font-size:18px">'+flag+'</span><div style="flex:1"><div style="font-weight:600;font-size:12px">'+(c.host||'?')+':'+c.port+'</div><div style="font-size:10px;color:#888">'+c.protocol?.toUpperCase()+' | '+c.latency+'ms</div></div><span class="badge badge-green">'+(i+1)+'</span></div>');}lines.push('</div>','<div style="background:#1a1a3e;border-radius:10px;padding:16px;border:1px solid #ef4444">','<div style="font-size:14px;font-weight:700;color:#ef4444;margin-bottom:8px">⚠️ سرورهایی که برای شما مناسب نیستند:</div>'];for(const c of worstConfigs){const flag=getFlag(c.country);lines.push('<div style="display:flex;align-items:center;gap:8px;padding:6px;border-bottom:1px solid #1a1a3e"><span style="font-size:18px">'+flag+'</span><div style="flex:1"><div style="font-weight:600;font-size:12px">'+(c.host||'?')+':'+c.port+'</div><div style="font-size:10px;color:#888">'+c.latency+'ms</div></div><span class="badge badge-red">❌</span></div>');}lines.push('</div>');out.innerHTML=lines.join('');}
